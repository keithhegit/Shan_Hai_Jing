...
Web 端 Three.js + Vue 3 的可交互体验站：MC 风格的多世界传送门探索 + “锁定选中目标 + 光束射线”的战斗预热体验。
...
- 基地模块：牧场终端 + 玩家建造围栏（Fence 模型 + 碰撞）+ 投放收容罐圈养（以当前实现为准）
...
- 天空表现：按 `main@7b7c7b8` 的实现方式回滚（背景贴图映射与相机 `far` 均恢复原状）
- 鱼系统：移除 FishNpcSystem 与相关资源/注册，避免“只有名字可见”的不稳定表现
- Hub 地形补丁：指定水域矩形区域填平成陆地（草地层）
- 旅行商人：缩放为原来的一半并贴地；新增 3x3 地台与实体碰撞阻挡；商店新增“卖出”流程
- 牧场建造：Fence 恢复为 GLTF 围栏模型，并补齐碰撞（玩家与小动物均不可穿过）；`Fence_Center ≤ 16`，`Fence_Corner ≤ 4`
- 锁定镜头：取消锁定后的相机追踪（保持默认尾追视角），锁定仅用于确定射线目标
- 物品体验：鹤嘴镐图标路径修复并自动出现在快捷栏；`crystal_small/crystal_big` 改为堆叠物品
...
- 视觉链路：`src/js/camera/camera-rig.js`（默认尾追镜头）、`src/js/renderer.js`（后处理/暗角）、`src/components/PlayerHUD.vue`（锁定提示与血条）
