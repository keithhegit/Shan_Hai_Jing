...
- 修复：Boss 捕捉成功后也会生成地牢钥匙掉落（与击败一致）
- 修复：矿山地牢出口传送门恢复在出口点生成（可按 E 返回 Hub）
...
- 战斗与捕捉：已具备锁定（中键）、轻击（Z/X）、物质枪光束 DoT、敌人仇恨追击与近战攻击、Q 捕捉引导与收容罐掉落、击败/捕捉后目标清理等基础闭环；但“更完整的受击反馈/玩家死亡与重生/更多招式与判定精细化”仍需迭代
...
### 2.4 下一迭代入口（vNext：灵兽出战 / 投掷收容罐召唤）

- 规格口径：见 `docs/pokemon_gdd.md` 的「vNext-0：灵兽出战（投掷收容罐召唤）」章节
- 关键入口：
  - 捕捉与掉落：`src/js/world/systems/capture-system.js`、`src/js/world/systems/drop-system.js`
  - 背包/仓库：`src/js/world/systems/inventory-system.js`（需要支持“收容罐实例元数据”）
  - 敌人与动物更新：`src/js/world/systems/dungeon-enemy-system.js`、`src/js/world/systems/hub-npc-system.js`
  - 输入与投掷：`src/js/utils/input.js`（F 与右键）、`src/js/world/world.js`（现有举高/掷出链路可复用）
  - UI 与日志：`src/components/PlayerHUD.vue`、`src/js/utils/event-bus.js`
- 最大风险点（需要先对齐）：现有收容罐只按 `itemId` 区分大小，不携带“抓到的灵兽是谁”的信息；vNext 要实现“抓到什么放出什么”，需要把收容罐升级为可携带元数据的实例物品（并保证存档兼容）
