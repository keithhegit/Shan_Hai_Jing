...
- 抓捕结果目前是“直接入背包”，还缺少“地面生成可拾取实体战利品”的环节
- Canister 目前只区分 Small/Medium/Large 三个道具 ID，没有区分“Boss 罐（超大占格）”
- 捕捉过程缺少进度 UI（读条/取消提示），失败原因也没有可视化
