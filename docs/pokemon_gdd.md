...
3. 自动关闭背包（不要求再按 ESC）
4. 主控进入“抱起待投掷”状态：HUD 显示“右键投掷”（与对 NPC 按 F 抱起一致的反馈）
...
- 环境视觉：Terrain 方块从默认 Box/材质替换为 `public/models/cube` 的 gltf；Hub 超平坦树替换为 `Tree_1/2/3.gltf`，按区域随机混用
