...
- Boss 奖励：无论击败还是捕捉 Boss，都会生成地牢钥匙掉落（掉落点与原规则一致）
- 矿山地牢：出口传送门恢复在出口点生成，可按 E 返回 Hub
- Hub：仓库模型挪到石头位置并放大到 200%，更显眼
...
#### vNext-0：灵兽出战（投掷收容罐召唤）

目标：让“捕捉到的灵兽”真正参与战斗。玩家可以像宝可梦一样投掷收容罐，将灵兽放出协助主控作战；灵兽被打到 0 HP 后，会恢复成“可拾取的收容罐”形态，等待主控回收。

玩家操作（复用现有输入与手感）：

- 从背包选中一个收容罐（`canister_small / canister_medium / canister_large`）
- 按 F：进入“举高投掷”准备态（复用当前“举高”语义）
- 鼠标右键：投掷收容罐到准星落点（复用当前“右键掷出”语义）
- 收容罐落地后：生成 1 个“已出战灵兽实体”，并把该收容罐从背包移除

战斗与仇恨规则（最小可用口径）：

- 敌人目标选择优先级：最近的“出战灵兽” > 主控
- 当附近没有出战灵兽（或都在攻击范围/仇恨范围外）时，敌人才追赶主控
- 出战灵兽的 HP/攻击/移速：第一版复用当前同类型 NPC/敌人的数值（不新增数值表字段）
- 出战灵兽 HP 归零：实体消失，生成一个“可拾取收容罐掉落物”（回收到背包）

数据前置（本迭代最关键的结构性工作）：

- 现状：收容罐道具只有 `itemId`，没有“这只灵兽是谁”的信息
- vNext 需要：把收容罐升级为“可携带元数据的实例物品”，至少包含：
  - `capturedResourceKey`（例如 `animal_wolf` / `enemy_orc` / `enemy_giant`）
  - `capturedKind`（`npc|minion|boss`，用于规则与 UI 兜底）
  - `capturedDisplayName`（用于日志与 UI 提示）
- 约束：不要求新增“稀有个体/IV”等系统；只要能保证“抓到什么就放出什么”

验收口径（最小）：

1. 玩家背包里有收容罐时，可按 F 进入举高态，并能右键投掷到准星落点
2. 投掷后会生成 1 个灵兽实体（模型正确，能跟随/能攻击），同时背包内该罐被消耗
3. 敌人优先攻击身边灵兽；当灵兽被打死后敌人才会重新追主控
4. 灵兽 HP 归零后，地面生成可拾取的收容罐；拾取后可回到背包
5. 退出地牢/返回 Hub 时，不残留“出战灵兽实体”（全部清理或回收为掉落）
