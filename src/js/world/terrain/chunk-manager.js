...
export default class ChunkManager {
  ...
  removeTreesInWorldBoxes(boxes) {
    ...
    let removed = 0
    for (const chunk of this.chunks.values()) {
      const treeData = chunk?.generator?.treeModelData
      ...
      if (next.length !== treeData.length) {
        chunk.generator.treeModelData = next
        chunk.treeModelRenderer?.build?.(next)
      }
    }
  ...
  removeTreesInWorldRadius(centerX, centerZ, radius) {
    const cx = Number(centerX)
    const cz = Number(centerZ)
    const r = Math.max(0, Number(radius) || 0)
    if (!Number.isFinite(cx) || !Number.isFinite(cz) || !(r > 0))
      return 0
    return this.removeTreesInWorldBoxes([{
      minX: cx - r,
      maxX: cx + r,
      minZ: cz - r,
      maxZ: cz + r,
